...
const ChatsItem = ({ data, isOnline, onClick }) => {
    ...
    const getFileIcon = (file) => {
        const { type } = file;
        if (type.startsWith('image')) return <FaFileImage className="text-gray-500" />;
        if (type === 'application/pdf') return <FaFilePdf className="text-white" />;
        if (type.startsWith('audio')) return <FaFileAudio className="text-white" />;
        if (type.startsWith('video')) return <FaVideo className="text-white" />;
    };
